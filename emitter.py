...
def generate_header_file(input_filename: str, generator_description: GeneratorDescription, namespace: str) -> str:
    ...
    new_line = '\n'
    result += f"    {generator_description.postlude.replace(new_line, new_line + '    ')}\n\n"
...
def forward_declarations(generator_description: GeneratorDescription) -> str:
    ...
    for key, value in generator_description.abstract_types.items():
        result += f"    struct {key};\n"
        for type_ in value.sub_types.keys():
